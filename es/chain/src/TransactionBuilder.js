...
var head_block_time_string, committee_min_review, maximum_proposal_lifetime;
...
var TransactionBuilder = function () {
    ...
    */
    ...
    TransactionBuilder.prototype.get_type_operation = function get_type_operation(name, operation) {
        ...
        assert(operation, "operation");
        var _type = ops[name];
        assert(_type, "Unknown operation 1" + name);
        var operation_id = ChainTypes.operations[_type.operation_name];
        if (operation_id === undefined) {
            throw new Error("unknown operation 2: " + _type.operation_name);
        }
        if (!operation.fee) {
            operation.fee = { amount: 0, asset_id: 0 };
        }
        if (name === 'account_create') {
            if (operation.options && !operation.options.auth_key) {
                operation.options.auth_key = operation.options.memo_key
            }
        }
        if (name === 'account_update') {
            if (operation.new_options && !operation.new_options.auth_key) {
                operation.new_options.auth_key = operation.new_options.memo_key
            }
        }
        //特殊处理 need_auth
        if (name === 'carrier_create' || name === 'carrier_update' || name === 'gateway_create' 
            || name === 'gateway_update' || name === 'finance_option_create' || name === 'finance_option_update'
            || name === 'account_authenticate') {
            operation.need_auth || (operation.need_auth = 0)
        }
            ...
            */
            ...
            operation.proposed_ops.forEach(function (op) {
                ...
                switch (op.op[0]) {
                    ...
                    case 5:
                        if (op.op[1].options) {
                            op.op[1].options.auth_key = op.op[1].options.memo_key
                        }
                        break;
                    case 6: //account_update
                        key = "account";
                        if (op.op[1].new_options) {
                            op.op[1].new_options.auth_key = op.op[1].new_options.memo_key
                        }
                        break;
                    case 17:
                        // asset_settle
                    ...
                    case 79:
                    case 100:
                        op.op[1].need_auth || (op.op[1].need_auth = 0)
                        break;
            ...
            operation.expiration_time || (operation.expiration_time = base_expiration_sec() + maximum_proposal_lifetime - 100);
            operation.review_period_seconds =committee_min_review + 1000
            // if (requiresReview) {
    ...
    TransactionBuilder.prototype.update_head_block = function update_head_block() {
        return Apis.instance().db_api().exec("get_objects", [["2.0.0", "2.1.0"]]).then(function(res) {
            var g = res[0],
                r = res[1];

            head_block_time_string = r.time;
            committee_min_review = g.parameters.committee_proposal_review_period;
            maximum_proposal_lifetime = g.parameters.maximum_proposal_lifetime;
        })
        ...
        //     maximum_proposal_lifetime = g[0].parameters.maximum_proposal_lifetime;
        // });
    };
    ...
    TransactionBuilder.prototype.set_required_fees = function set_required_fees(asset_id) {
        ...
        if (asset_id !== "1.3.0") {
            // This handles the fallback to paying fees in ZOS if the fee pool is empty.
    ...
    TransactionBuilder.prototype.sign = function sign() {
        ...
        for (var i = 0; 0 < end ? i < end : i > end; 0 < end ? i++ : i++) {
            ...
            // console.log(this.tr_buffer) //guo
    ...
    TransactionBuilder.prototype.broadcast = function broadcast(was_broadcast_callback) {
        ...
        if(tr_object.operations[0][0] === 22) {
            return Apis.instance().db_api().exec("validate_proposal",[tr_object]).then(function () {
                if (_this4.tr_buffer) {
                    return _this4._broadcast(was_broadcast_callback);
                } else {
                    return _this4.finalize().then(function () {
                        return _this4._broadcast(was_broadcast_callback);
                    });
                }
...
function _broadcast(was_broadcast_callback) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        // Apis.instance().network_api().exec("get_transaction_hash",[tr_object])  //guo
        ...
        Apis.instance().network_api().exec("broadcast_transaction_with_callback", [function (res) {
            ...
            reject(new Error(message + "\n" + 'zos-crypto ' + ' digest ' + hash.sha256(_this5.tr_buffer).toString('hex') + ' transaction ' + _this5.tr_buffer.toString('hex') + ' ' + JSON.stringify(tr_object)));
            return;
        });
        return;
    });
