...
var budget_member_object_type = parseInt(object_type.budget_member, 10);
var account_object_type = parseInt(object_type.account, 10);
var asset_object_type = parseInt(object_type.asset, 10);
var gateway_object_type = parseInt(object_type.gateway, 10);
var carrier_object_type = parseInt(object_type.carrier, 10);
...
var budget_prefix = "1." + budget_member_object_type + ".";
var asset_prefix = "1." + asset_object_type + ".";
var account_prefix = "1." + account_object_type + ".";
var gateway_prefix = "1." + gateway_object_type + ".";
var carrier_prefix = "1." + carrier_object_type + ".";

var loginAccount = '';
var loginBalancesChange  = false;
var loginBalancesChange2 = false;
var loginCouponChange = false;
var loginCoupon = 10000000;
...
var ChainStore = (function() {
    ...
    ChainStore.prototype.clearCache = function clearCache() {
        ...
        */
        ...
        this.subbed_budget = new Set();
        this.subbed_gateway = new Set();
        this.subbed_carrier = new Set();
        ...
        this.budget_by_account_id = new Map();
        this.gateway_by_account_id = new Map();
        this.carrier_by_account_id = new Map();
        ...
        this.next_maintenance_time = 0
        this.next_maintenance_time_ = true
    };
    ...
    ChainStore.prototype.init = function init() {
        ...
        var _init = function _init(resolve, reject) {
            ...
            _this.next_maintenance_time_ = true
            return db_api
                ...
                .then(function(optional_objects) {
                    ...
                    for (var i = 0; i < optional_objects.length; i++) {
                        ...
                        */
                            ...
                            _this.next_maintenance_time = optional_object.next_maintenance_time
    ...
    ChainStore.prototype.getObject = function getObject(id) {
        ...
        if (result === undefined || force || subChange)
            return this.fetchObject(id, force, autosubscribe, no_full_account, is_history);
    ...
    ChainStore.prototype.fetchObject = function fetchObject(id) {
        ...
        if (id.indexOf("1.2.") === 0 && !no_full_account)
            return this.fetchFullAccount(id, autosubscribe);
        if (id.indexOf(witness_prefix) === 0) this._subTo("witnesses", id);
        if (id.indexOf(committee_prefix) === 0) this._subTo("committee", id);
        if (id.indexOf(budget_prefix) === 0) this._subTo("budget", id);
        if (id.indexOf(gateway_prefix) === 0) this._subTo("gateway", id);
        if (id.indexOf(carrier_prefix) === 0) this._subTo("carrier", id);
            ...
            if (!is_history) {
                ...
                Apis.instance()
                    ...
                    .then(function(optional_objects) {
                        ...
                        // }
                        if (optional_objects)
                            _this7._updateObject(optional_objects, true);
                        else {
                            _this7.objects_by_id.set(id, null);
                            _this7.notifySubscribers();
                        }
    ...
    ChainStore.prototype.getBudgetMemberById = function getBudgetMemberById(
        account_id
    ) {
        var cm_id = this.budget_by_account_id.get(account_id);
        if (cm_id === undefined) {
            this.fetchBudgetMemberByAccount(account_id);
            return undefined;
        } else if (cm_id) {
            this._subTo("budget", cm_id);
        }
        return cm_id ? this.getObject(cm_id) : null;
    };

    ChainStore.prototype.getGatewayById = function getGatewayById(
        account_id
    ) {
        var cm_id = this.gateway_by_account_id.get(account_id);
        if (cm_id === undefined) {
            this.fetchGatewayByAccount(account_id);
            return undefined;
        } else if (cm_id) {
            this._subTo("gateway", cm_id);
        }
        return cm_id ? this.getObject(cm_id) : null;
    };

    ChainStore.prototype.getCarrierById = function getCarrierById(
        account_id
    ) {
        var cm_id = this.carrier_by_account_id.get(account_id);
        if (cm_id === undefined) {
            this.fetchCarrierByAccount(account_id);
            return undefined;
        } else if (cm_id) {
            this._subTo("carrier", cm_id);
        }
        return cm_id ? this.getObject(cm_id) : null;
    };
    ...
    /**
     *
     * @return a promise with the history workers array
     */

    ChainStore.prototype.fetchAllHistoryWorkers = function fetchAllHistoryWorkers(last_word_id) {
        var _this8 = this;

        return new Promise(function(resolve, reject) {
            Apis.instance()
                .history_api()
                .exec("get_workers_history", [last_word_id, 100])
                .then(function(workers_array) {
                    if (workers_array && workers_array.length) {
                        // workers_array.forEach(function(worker) {
                        //     _this8._updateObject(worker, false);
    ...
    /**
     *
     * @return a promise with the budget object
     */
    ChainStore.prototype.fetchBudgetMemberByAccount = function fetchBudgetMemberByAccount(
        account_id
    ) {
        var _this10 = this;
        ...
        return new Promise(function(resolve, reject) {
            Apis.instance()
                .db_api()
                .exec("get_budget_member_by_account", [account_id])
                .then(function(optional_budget_object) {
                    if (optional_budget_object) {
                        _this10._subTo(
                            "budget",
                            optional_budget_object.id
                        );
                        _this10.budget_by_account_id = _this10.budget_by_account_id.set(
                            optional_budget_object.budget_member_account,
                            optional_budget_object.id
                        );
                        var budget_object = _this10._updateObject(
                            optional_budget_object,
                            true
                        );
                        resolve(budget_object);
                    } else {
                        _this10.budget_by_account_id = _this10.budget_by_account_id.set(
                            account_id,
                            null
    ...
    ChainStore.prototype.getAssetList = function getAssetList(
        type
    ) {
        var _this10 = this;
        return new Promise(function(resolve, reject) {
            Apis.instance()
                .bitlender_api()
                .exec("get_asset_by_property", [type])
    ...
    ChainStore.prototype.getBalancesSortById = function getBalancesSortById(
        asset_id,
        start,
        limit
    ) {
        ...
        return new Promise(function(resolve, reject) {
            Apis.instance()
                .db_api()
                .exec("get_sort_balances", [asset_id,start,limit ])
                .then(function(balances) {                     
                   resolve(balances);                    
                }, reject);
        });
    ...
    ChainStore.prototype.fetchGatewayByAccount = function fetchGatewayByAccount(
        account_id
    ) {
        var _this10 = this;
        ...
        return new Promise(function(resolve, reject) {
            Apis.instance()
                .db_api()
                .exec("get_gateway_by_account", [account_id])
                .then(function(optional_gateway_object) {
                    if (optional_gateway_object) {
                        _this10._subTo(
                            "gateway",
                            optional_gateway_object.id
                        );
                        _this10.gateway_by_account_id = _this10.gateway_by_account_id.set(
                            optional_gateway_object.gateway_account,
                            optional_gateway_object.id
                        );
                        var gateway_object = _this10._updateObject(
                            optional_gateway_object,
                            true
                        );
                        resolve(gateway_object);
                    } else {
                        _this10.gateway_by_account_id = _this10.gateway_by_account_id.set(
                            account_id,
                            null
    ...
    ChainStore.prototype.fetchCarrierByAccount = function fetchCarrierByAccount(
        account_id
    ) {
        var _this10 = this;
        ...
        return new Promise(function(resolve, reject) {
            Apis.instance()
                .db_api()
                .exec("get_carrier_by_account", [account_id])
                .then(function(optional_carrier_object) {
                    if (optional_carrier_object) {
                        _this10._subTo(
                            "carrier",
                            optional_carrier_object.id
                        );
                        _this10.carrier_by_account_id = _this10.carrier_by_account_id.set(
                            optional_carrier_object.carrier_account,
                            optional_carrier_object.id
                        );
                        var carrier_object = _this10._updateObject(
                            optional_carrier_object,
                            true
                        );
                        resolve(carrier_object);
                    } else {
                        _this10.carrier_by_account_id = _this10.carrier_by_account_id.set(
                            account_id,
                            null
    ...
    ChainStore.prototype.setLoginAccount = function setLoginAccount(id) {
        loginAccount = id
        loginBalancesChange = true
        loginBalancesChange2 = true
        loginCouponChange = true
        loginCoupon = 1123456789
        //throw Error( `Argument is not an account name or id: ${name_or_id}` )
    };

    ChainStore.prototype.getLoginAccountBalanceChange = function getLoginAccountBalanceChange(type) {
        if (type == 0 ) {            
          var changed = loginBalancesChange
          loginBalancesChange = false
          return changed
        } else if (type == 1 ){
          var changed = loginCouponChange
          loginCouponChange = false
          return changed
        } else if (type == 2 ){
          var changed = loginBalancesChange2
          loginBalancesChange2 = false
          return changed
        } else {
          return false
        }       
    ...
    ChainStore.prototype._updateObject = function _updateObject(object) {
        ...
        */
        ...
        if (
            ...
            if (loginAccount === object.owner && loginCoupon != object.amout_coupon) {
                loginCouponChange = true;
                loginCoupon = object.amout_coupon;
            }
            ...
            object.id.substring(0, budget_prefix.length) == budget_prefix
        ) {
            // committee_member object
            if (!this._isSubbedTo("budget", object.id)) {
                return;
            }
        } else if (
            object.id.substring(0, gateway_prefix.length) == gateway_prefix
        ) {
            // committee_member object
            if (!this._isSubbedTo("gateway", object.id)) {
                return;
            }
        }  else if (
            object.id.substring(0, carrier_prefix.length) == carrier_prefix
        ) {
            // committee_member object
            if (!this._isSubbedTo("carrier", object.id)) {
                return;
            }
        } else if (
        ...
        if (object.id == "2.1.0") {
            ...
            if (this.next_maintenance_time !== object.next_maintenance_time){
                this.next_maintenance_time = object.next_maintenance_time
                this.next_maintenance_time_ = true 
            }
        ...
        if (object.id.substring(0, balance_prefix.length) == balance_prefix) {
            ...
            // COMMITTEE MEMBER OBJECT
            if (this._isSubbedTo("gateway", object.id)) {
                this.gateway_by_account_id.set(
                    object.gateway_account,
                    object.id
                );
                this.objects_by_vote_id.set(object.vote_id, object.id);
            } else {
            ...
            // COMMITTEE MEMBER OBJECT
            if (this._isSubbedTo("carrier", object.id)) {
                this.carrier_by_account_id.set(
            ...
            // BUDGET MEMBER OBJECT
            if (this._isSubbedTo("budget", object.id)) {
                this.budget_by_account_id.set(
                    object.budget_member_account,
                    object.id
                );
                this.objects_by_vote_id.set(object.vote_id, object.id);
            } else {
            ...
            //如果提案状态为: 2, 3，4, 或者过期时间大于now, 则不执行 addProposalData
            var head_date = new Date(this.head_block_time_string + "+00:00").getTime()
            var expiration_time = new Date(object.expiration_time + "+00:00").getTime()
            if (expiration_time > head_date && (object.status === 0 || object.status === 1)) {
    ...
    ChainStore.prototype.getBlockSummary = function getBlockSummary() {
        if (!this.next_maintenance_time_) {
            return Promise.resolve(undefined)
        } else {
           this.next_maintenance_time_ = false
           return Apis.instance().db_api().exec("get_block_summary", []).then(function(optional_objects) {
                  return Promise.resolve(optional_objects)
            }).catch(function(error) {                   
                console.log("!!! Chain API error", error);
